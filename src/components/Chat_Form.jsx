...
const Chat_Form = ({ input, setInput, files, setFiles, onSend }) => {
  ...
  return (
    <div className="px-4 py-3   bg-white shadow-md">
      <div className="flex items-center gap-3 bg-[#E8F1FF] border-[#E8F1FF] rounded-full px-4 py-2 shadow">
        ...
        {/* Attach */}
        <button onClick={() => fileRef.current.click()} className='cursor-pointer'>
          <BiPaperclip size={20} />
        </button>

        {/* Send / Mic */}
        {(input.trim() || files.length > 0) ? (
          <button onClick={onSend} className="bg-[#1E2235] text-white rounded-full p-3 cursor-pointer">
            <Send size={18} />
          </button>
        ) : (
          <button className="bg-gray-900 text-white rounded-full p-3 cursor-pointer">
            <Mic size={18} />
          </button>
        )}
      </div>
